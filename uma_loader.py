...
def init() -> None:
    ...
    # Deduplicate.
    unique_backend_classes = []
    for bcls in backend_classes:
        isDuplicate = False
        for ubcls in unique_backend_classes:
            if bcls.__name__ == ubcls.__name__:
                isDuplicate = True
        if not isDuplicate:
            unique_backend_classes.append(bcls)

    for backend_cls in unique_backend_classes:
        backend = backend_cls()
        backend.register()
